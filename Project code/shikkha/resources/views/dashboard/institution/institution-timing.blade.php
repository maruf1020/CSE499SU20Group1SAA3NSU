			...
			<div class="card card-custom">
				...
				<div class="card-header py-3">
					<div class="card-title align-items-start flex-column">
						<h3 class="card-label font-weight-bolder text-dark">Timing Information</h3>
						<span class="text-muted font-weight-bold font-size-sm mt-1">Timing and there information</span>
					</div>
						...
						<div class="modal fade" id="exampleModalCenter" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdrop" aria-hidden="true">
							<div class="modal-dialog modal-dialog-centered" role="document">
								<div class="modal-content">
									...
									<form class="" action="{{ route('timing.store') }}" method="post">
										<div class="modal-body">
											...
											<div class="form-group">
												...
												<input name="start" type="time" class="form-control form-control-lg" placeholder="Write timing Name"></div>
											<div class="form-group">
												<label>End Time</label>
												<input name="end" type="time" class="form-control form-control-lg" placeholder="Write Phone Number ">
                                            </div>
									...
									<form id="update-form" action="" method="post">
										...
										<div class="modal-body">
											...
											@method('PUT')
                                                ...
                                                <div class="form-group">
                                                    <label>Start Time</label>
                                                    <input id="start" name="start" type="time" class="form-control form-control-lg" placeholder="Write timing Name"></div>
                                                <div class="form-group">
                                                    <label>End Time</label>
                                                    <input id="end" name="end" type="time" class="form-control form-control-lg" placeholder="Write Phone Number ">
                                                </div>
											...
											<tbody>
												...
												<tr class='value{{ $value->id }}'>
													...
													<td>
                                                        ...
                                                        $finalhours = $hours1-$hours;
                                                        $finalminutes = $minutes1-$minutes;
                                                        if($finalhours == 0)
                                                            echo $finalhours.':'.$finalminutes .' Minutes';
                                                        elseif ($finalhours == 1)
                                                            echo $finalhours.':'.$finalminutes .' Hour';
                                                        else
                                                            echo $finalhours.':'.$finalminutes .' Hours';
													...
													<td>
														<a href="javascript:;" data-toggle="modal" data-target="#edit" class="btn btn-sm btn-clean btn-icon mr-2 edit" title="Edit details" data-id='{{ $value->id }}' data-name='{{ $value->name }}'
														  data-start='{{ $value->start }}' data-end='{{ $value->end }}' data-initial='{{ $value->initial }}' data-dob='{{ $value->dob }}' data-address='{{ $value->address }}'>
															<i class="far fa-edit"></i>
														</a>

														<a href="javascript:void(0)" data-method="DELETE" data-token="{{csrf_token()}}"  data-url="{{ route('timing.destroy',$value->id) }}" data-id="{{ $value->id }}" class="btn btn-sm btn-clean btn-icon delete" title="Delete">
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$(document).on('click', '.edit', function() {
				...
				// $('#update-form').attr('action',"/timing/"+id);
			});
			...
			$.ajax({
				...
				success: function(data) {
					$('.value' + data.id).replaceWith(" " +
						...
						"<td>" + data.end + "</td>" +
						"<td>" + data.initial + "</td>" +
						"<td><a href='javascript:;' data-toggle='modal' data-target='#edit' class='btn btn-sm btn-clean btn-icon mr-2 edit' title='Edit details' data-id='"+data.id+"' data-start='"+data.start+"' data-end='"+data.end+"' data-email='"+data.email+"' data-initial='"+data.initial+"' data-dob='"+data.dob+"' data-address='"+data.address+"'>"+
					...
					Swal.fire({
						title: "Success!",
						text: "timing Updated Succesfully!",
						icon: "success",
