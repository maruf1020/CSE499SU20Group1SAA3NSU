			...
			<div class="card card-custom">
				...
				<div class="card-header py-3">
					<div class="card-title align-items-start flex-column">
						<h3 class="card-label font-weight-bolder text-dark">session Information</h3>
						<span class="text-muted font-weight-bold font-size-sm mt-1">session and there information</span>
					</div>
					<div class="card-toolbar">
						...
						<!--end::Button-->
						<!--begin::Button-->
						<a href="{{ route('previous-session') }}" class="btn btn-success font-weight-bolder mr-2">
							<span class="svg-icon svg-icon-md">
								<!--begin::Svg Icon | path:/metronic/theme/html/demo2/dist/assets/media/svg/icons/Design/Flatten.svg-->
								<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
									<g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
									...
									<form class="" action="{{ route('session.store') }}" method="post">
										<div class="modal-body">
											...
											<div class="form-group">
												...
												<input name="name" type="text" class="form-control form-control-lg" placeholder="Write session Name"></div>
											<div class="form-group">
												<label>Session Start</label>
												<input name="start" type="date" class="form-control form-control-lg" placeholder="Write Phone Number "></div>
											<div class="form-group">
												<label>Session End</label>
												<input name="end" type="date" class="form-control form-control-lg" placeholder="Write Email">
											</div>
											<div class="form-group">
								...
								<div class="modal-content">
									...
									<form id="update-form" action="" method="post">
										...
										<div class="modal-body">
											...
											<div class="form-group">
												<label>Session name</label>
												<input id="name" name="name" type="text" class="form-control form-control-lg" placeholder="Write session Name">
											</div>
											<div class="form-group">
												<label>Session Start</label>
												<input id="start" name="start" type="date" class="form-control form-control-lg" placeholder="Write Phone Number ">
											</div>
											<div class="form-group">
												<label>Session End</label>
											...
											<div class="form-group">
												<label>If you want End the enitre Session (Higly restricted)</label>
												<select id="stop" class="form-control form-control-lg" name="status">
												...
												<tr class='value{{ $value->id }}'>
													...
													<td>
														<a href="javascript:;" data-toggle="modal" data-target="#edit" class="btn btn-sm btn-clean btn-icon mr-2 edit" title="Edit details" data-id='{{ $value->id }}' data-name='{{ $value->name }}'
														  data-start='{{ $value->start }}' data-end='{{ $value->end }}' data-status='{{ $value->status }}'>
															<i class="far fa-edit"></i>
														</a>

														<a href="javascript:void(0)" data-method="DELETE" data-token="{{csrf_token()}}" data-url="{{ route('session.destroy',$value->id) }}" data-id="{{ $value->id }}"
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$(document).on('click', '.edit', function() {
				...
				// $('#update-form').attr('action',"/session/"+id);
			});
			...
			$.ajax({
				...
				success: function(data) {
					$('.value' + data.id).replaceWith(" " +
						...
						"<td>" + data.end + "</td>" +
						"<td>" + data.status + "</td>" +
						"<td><a href='javascript:;' data-toggle='modal' data-target='#edit' class='btn btn-sm btn-clean btn-icon mr-2 edit' title='Edit details' data-id='" + data.id + "' data-name='" + data.name + "' data-start='" +
						data.start + "' data-end='" + data.end + "' data-status='" + data.status + "' data-dob='" + data.dob + "' data-address='" + data.address + "'>" +
					...
					Swal.fire({
						title: "Success!",
						text: "session Updated Succesfully!",
						icon: "success",
